function Asenac(html){
  var mime = html.indexOf("xmlns=") == -1 ? "text/html" : "image/svg+xml";   
  var parsed= Asenac.parser.parseFromString(html, mime);
  return mime=="text/html" ? parsed.body.firstChild : parsed.firstChild;
}
Asenac.parser = new DOMParser();

/** @param {string} code */
Asenac.Build = function(code){
    ...
    for(let i=0;i<stack.length;i++){
      ...
      if(mat && row.indexOf("<") < row.indexOf(">") || (mat && row.split(" ").join("").indexOf("=><")>-1 ) ){
        ...
        if(prefix.trim()=="return"){
          row += Space(line.indent+1) + prefix + 'Asenac(`'+html+'`);\n'
        }else if(prefix.trim().endsWith("=>")){
          row += Space(line.indent+1) + prefix + 'Asenac(`'+html+'`);\n'
        }else{
          row += Space(line.indent+1) + 'let that=' + (isParentAdded?"":"PARENT=") + prefix + 'Asenac(`'+html+'`);\n'
        }
